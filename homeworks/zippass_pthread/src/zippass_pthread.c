  ...
  */
  ...
  for (uint64_t i = 0; i < txt_file.num_of_zip_files; i++) {
    generate_zip_password(&txt_file.max_password_length, txt_file.alphabet,
                          txt_file.zip_files_directions[i], number_threads);
  }
...
void generate_zip_password(uint64_t* password_length, const char* alphabet,
                           const char* zip_dir, const uint64_t num_threads) {
  //  declare num_position :=0
  //  declare password_generated[password_length];
  //  declare password_temp[]
  //  declare is_password_found

  uint64_t pass_length = 0;
  ...
  // https://stackoverflow.com/questions/23044184/c-or-c-combination-with-repetition
  while (pass_length <= *password_length && generate_more_password) {
    ...
    while (alphabet_index < total_posible_combination &&
      ...
      for (uint64_t k = 0; k < pass_length; k++) {
